...
Получился вот такой вот интерфейс программы.
...
При создании кодировщика на Python дыла использована та же форма, что и для c++ и полностью сохранен алгоритм и функционал программы на c++. Внешне оба интерфейса ничем не отличаются за исключением того, что программа на Python работает в разы медленнее, чем на c++ (особенно это заметно на больших файлах).
При создании программы на Python я выбрал вариант с преобразованием шаблона пользовательской формы в отдельный класс - так мне показалось удобней.

Ниже привожу полный текст программы на Python:
```python
# -*- coding: utf-8 -*-
...
<br>
Внутри предоставленного для тестирования файла оказался аудиотрек и анимированный jpeg Риком Астлей, который почему-то обещал нам Grand Theft Auto VI.

<h2>Тестовая картинка</h2>
&nbsp;&nbsp;&nbsp;&nbsp;<b>К информации в сообщении стоит относиться с юмором!</b>
&nbsp;&nbsp;&nbsp;&nbsp;В тестовой картинке содержится секретная информация о работе некоторых студентов над курсовой работой.
<p align="center"><img src="image/06.png"></p>
<p align="center">Рис 6. В этой картинке содержится сообщение.</p>
<br>
<br>
Кроме того, я подготовил ещё несколько тестовых [[картинок]](./05/minitests), которые проверяют проверить корректность работы кодировщика на нестандартных размерах картинки: от 18 до 27 пиксел.
